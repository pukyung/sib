...
<html>
	...
	<body class=" hanhobody">
		...
		<div>
			<input type="button" onclick="location.href='eventcheck.pj'" value="당첨 확인" />
		</div>
		...
		<div>
			<div>
				<table>
					...
					<tbody>
						<q:forEach items="${ eventlist }" var="t" varStatus="count" >
							<form method="post" action="eventdetail.pj" id="form${ t.evnum }" >
								<input type="hidden" name="pid" value="${ t.pid }" />
								<input type="hidden" name="eprice" value="${ t.eprice }" />
								<input type="hidden" name="date" value="${ t.ddate }" />
								<input type="hidden" name="evnum" value="${ t.evnum }" />
								<tr onclick="document.getElementById('form' + ${ t.evnum }).submit();">
									<td>${ count.count }</td>
									<td>${ t.pname }</td>
									<td>${ t.ddate }</td>
								</tr>
